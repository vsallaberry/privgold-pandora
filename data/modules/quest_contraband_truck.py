...
class quest_contraband_truck (quest.quest):
    ...
    def mission_fail(self):
        ...
        debug.debug("mission terminating")

        debug.debug("mission terminated")
        self.quest_exit = 1
    ...
    def start_destruction(self):
        if self.repeat_end2 == 2 and not self.cargo_container.isNull():
            ...
            debug.debug("ended msgs")
            self.jumpout = 1
            self.repeat_end2 = 0
            debug.debug("ended start_destruction")

    def end_destruction(self):
#               debug.debug("testing timer")
        if (self.timer1 + 12) <= VS.GetGameTime():

            debug.debug("playing sounds")
...
#                       debug.debug("Truck has Jumped Away :-)")
#               else:
#                       debug.debug("Truck is still here! :-(")
        ...
        if (self.playa):
            ...
            if self.truck_pirate.getMinDis(self.playa.Position()) < 200:
                ...
                if self.repeat_end1 == 1:
                    ...
                    debug.debug("My target is...")

                    global truck_exit
                    truck_exit = 1
                    debug.debug(repr(truck_exit))

						...
						*size=ChunkHdr.Size;
						if(*data == NULL){
							*data=malloc(ChunkHdr.Size + 31);
							...
							ALvoid * newdata = realloc(*data,ChunkHdr.Size + 31);
							if (newdata == NULL) free(*data);
							*data = newdata;
						}
						if (*data) 
						{
...
#ifdef HAVE_AL
  if (sound>=0&&sound<(int)sounds.size()) {
    float p []= {static_cast<float>(scalepos*pos.i),static_cast<float>(scalepos*pos.j),static_cast<float>(scalepos*pos.k)};
    float v []= {static_cast<float>(scalevel*vel.i),static_cast<float>(scalevel*vel.j),static_cast<float>(scalevel*vel.k)};

...
static FILE * withAndWithout (std::string filename, std::string time_of_day_hint) {
  std::string with (filename+"_"+time_of_day_hint+BASE_EXTENSION);
  FILE * fp = VSFileSystem::vs_open (with.c_str(),"r");
  if (!fp) {
    std::string without (filename+BASE_EXTENSION);
...
void BaseInterface::Load(const char * filename,const char * time_of_day_hint, const char * faction) {
  ...
  std::string full_filename = std::string("bases/") + filename;
  ...
  if(!inFile) {
	...
	daynight_filename = std::string("bases/planet_")+time_of_day_hint+string(BASE_EXTENSION);
	...
	daynight_filename = std::string("bases/unit_")+time_of_day_hint+string(BASE_EXTENSION);
	inFile = VSFileSystem::vs_open (daynight_filename.c_str(),"r");
	newfile=daynight_filename;
	if (!inFile) {
...
#else
  FILE * inFile = getFullFile (std::string("bases/")+filename,time_of_day_hint,faction);
  if (!inFile){
    bool planet=false;
    Unit *baseun=this->baseun.GetUnit();
    if (baseun) {
    ...
    std::string basestring ("bases/unit");
    if (planet) {
  ...
  std::string compilefile = std::string(filename)+time_of_day_hint+std::string(faction)+BASE_EXTENSION;

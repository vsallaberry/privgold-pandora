...
[Currently supported versions are 1.28,1.35,1.50, and system]))
case "$with_boost" in
   "") with_boost=1.35 ;;
   "1.28" | "1.35" | "1.50" | "system") ;;
...
if (test "x${with_boost}" = "x1.35") || (test "x${with_boost}" = "x1.50"); then
...
AM_CONDITIONAL([BOOST1_50], [test x$with_boost = x1.50])
